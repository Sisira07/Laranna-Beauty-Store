...
def click_bind(e):
    if cval.get()=='3oz                                                  ':
        price_label.config(text='$45.00')
    elif cval.get()=='1.2oz                                                   ':
        price_label.config(text='$30.00')
...
menu= Label(frame2,bg='dark slate blue', width=root.winfo_screenwidth(),height=3, bd=0,highlightthickness=0, autostyle=False)
...
MenuBttn5=Button(my_frame, text = "  CAREERS  ", font=('Times New roman',12),relief = RAISED,
                 ...
                 command=finalbill, autostyle=False)
...
#Photoslider
pics_list=['serum1.jpg', 'serum.jpg','serum2.jpg']
...
sshead=Image.open('serumhead.png')
sstk=ImageTk.PhotoImage(sshead)
sslab=Label(my_frame, image=sstk, bd=0, highlightthickness=0,autostyle=False)
sslab.place(x=545,y=120)
...
price_label=Label(my_frame, text='$30.00', font=('Microsoft Yahei UI Light', 22,'bold'), bg='white', fg='black', autostyle=False)
...
canvas=Canvas(my_frame,bg='lavender',width=root.winfo_screenwidth(),height=50,bd=0, highlightthickness=0,autostyle=False)
...
insert_text1= """
- Gel-to-liquid-to-powder technology or easy absorption
